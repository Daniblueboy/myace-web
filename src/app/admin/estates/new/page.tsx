...
export default function NewEstatePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await fetchAPI('/admin/estates', {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        body: JSON.stringify({
          ...form,
          gallery: form.gallery ? form.gallery.split(',').map((item) => item.trim()).filter(Boolean) : [],
      ...
      await showAlert({ title: 'Failed to create estate' });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold tracking-tight">Add Estate</h2>
